...
async function display() {
    ...
    let card_c = document.querySelector(".card_c")
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs/")) {
            ...
            let r = await a.json();
            card_c.innerHTML = card_c.innerHTML + ` <div data-folder="${folder}" class="card">
    ...
    // loading the palylist to get the songs
...
async function main() {
    ...
    document.querySelector(".seekbar").addEventListener("click", e => {
        ...
        // duration update whle moving the seekbar
    })

    // Add an event listener for menu
    document.querySelector(".menu").addEventListener("click", () => {
        document.querySelector(".left").style.left = "0"
        // to make the left appear
    })

    // Add an event listener for closing the menu
...
main()
// ideally we get all the songs from server using backend
